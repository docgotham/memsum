-- Fixed-window rate limiting for the kernel's MCP and OAuth surfaces. One
-- self-resetting row per key: the upsert either increments the current
-- window's counter or starts a new window, so no cleanup job is required and
-- the table stays bounded by distinct-key cardinality. The function is
-- service-role only — limits are enforced by the kernel in front of the data
-- layer, never consulted by browsers, and the kernel fails open if this
-- function is unreachable (rate limiting must not take the product down).
...
begin
  if auth.role() <> 'service_role' then
    raise exception 'check_rate_limit is service-role only';
  end if;
