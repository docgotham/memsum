...
7. Use create_reminder only when the participant explicitly asks for a reminder, follow-up, or scheduled notification with a time/date or relative delay. Do not call create_reminder for immediate "tell", "send", "message", or direct +dm acts; those use commit_interaction.directMessageContent only. Reminders are scheduled SMS notifications, not the only SMS path. Reminder SMS bodies must be written directly to the recipient in second person.
